...
class HomePatientSuperAdminController
    extends StateNotifier<HomePatientSuperAdminState> {
  ...
  HomePatientSuperAdminController(super._state,
  ...
  //Note SOAP
  String? noteS =
      "Hábitos de fumar, alcohol, mal dormir, comidas a destiempo. Hábitos de fumar, alcohol, mal dormir, comidas a destiempo. Hábitos de fumar, alcohol, mal dormir, comidas a destiempo.";
  ...
  Future<void> init() async {
    ...
    await listDoctorsSpecialist(patientId: patientId);
    await loadReports();
    await loadRecipe();
  }
  ...
  Future<void> listDoctorsSpecialist(
      {DoctorSpecialistState? doctorSpecialistState, patientId}) async {
    if (doctorSpecialistState != null) {
      state = state.copyWith(doctorSpecialistState: doctorSpecialistState);
    }
    ...
    state = result.when(
      left: (failure) {
        ...
        return state.copyWith(
            doctorSpecialistState: DoctorSpecialistState.failed(failure));
      },
      right: (list) {
        listD = list;
        return state.copyWith(
            doctorSpecialistState: DoctorSpecialistState.loaded(list));
      },
    );
  }
    ...
    state.viewState.when(
        ...
        historicClinic: (historic) {
          historic.when(
              dataBasic: () {
                state = state.copyWith(
                    isSettingDataBasic: !state.isSettingDataBasic);
              },
              dataPathology: () {
                state = state.copyWith(
              ...
              dataHabit: () {
                state = state.copyWith(
                    isSettingDataHabit: !state.isSettingDataHabit);
              },
              dataFallowed: () {},
              dataSpecialist: () {},
              dataSO: () {},
  ...
  Future<void> searchOnChanged(String text) async {
    state = state.copyWith(
        doctorSpecialistState: const DoctorSpecialistState.loading());
    ...
    state = state.copyWith(
        doctorSpecialistState: DoctorSpecialistState.loaded(displayList));
  }
}
