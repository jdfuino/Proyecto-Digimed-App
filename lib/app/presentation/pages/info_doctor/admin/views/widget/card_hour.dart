...
class CardHours extends StatelessWidget {
  const CardHours({super.key});

  @override
  Widget build(BuildContext context) {
    final InfoDoctorAdminController controller = Provider.of(context);
    return Container(
      width: double.infinity,
      margin: const EdgeInsets.only(right: 24, left: 24),
      child: controller.state.doctorDataState.when(loading: () {
        return const Center(
        ...
        return GestureDetector(
          onTap: (){
            popAndPushNewPageResumeHours(context,user.id);
          },
          child: CardDigimed(
            child: Container(
              margin:
