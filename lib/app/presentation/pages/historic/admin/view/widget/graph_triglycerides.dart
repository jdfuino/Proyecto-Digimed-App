...
class GraphTriglycerides extends StatefulWidget {
  final String title;
  ...
  @override
  State<GraphTriglycerides> createState() => _GraphTriglyceridesState();
}

class _GraphTriglyceridesState extends State<GraphTriglycerides>
  ...
  Map<String, double> _calculateDynamicRange() {
    ...
    if (dataMin == double.infinity || dataMax == double.negativeInfinity) {
      dataMin = 0; // Para triglicéridos no hay mínimo
    ...
    // Para triglicéridos, el mínimo siempre es 0
    double actualMin = 0;

    // Aplicar márgenes del 10% basándose en los valores reales
    double finalMax = actualMax + (actualMax * 0.1);
    double finalMin = 0; // Siempre empezar desde 0 para triglicéridos
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Flexible(
                  flex: 3,
                  child: controller.state.triglyceridesDataState.when(
    ...
    return PopupMenuButton<String>(
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Text(
              controller.valueSelectedTriglycerides,
  ...
  Widget _buildEnhancedChart(HistoricPatientsController controller) {
    ...
    if (_lastSelectedTime != controller.valueSelectedTriglycerides) {
      _lastSelectedTime = controller.valueSelectedTriglycerides;
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        return SizedBox(
          ...
          child: Stack(
            children: [
              // Solo línea gruesa verde para marcar el límite máximo de triglicéridos - DETRÁS del gráfico
              if (widget.maxSafeZone >= chartMinY && widget.maxSafeZone <= chartMaxY)
                Positioned(
                  left: 0,
                  right: 0,
                  ...
                  barTouchData: BarTouchData(
                    ...
                    touchTooltipData: BarTouchTooltipData(
                      ...
                      getTooltipItem: (group, groupIndex, rod, rodIndex) {
                        ...
                        String timeRange = controller.getTriglyceridesTimeRangeForPoint(groupIndex);
  ...
  double _calculateNormalZoneTop(double chartMinY, double chartMaxY) {
    // Para triglicéridos, la zona normal va desde chartMinY hasta maxSafeZone
  ...
  double _calculateNormalZoneHeight(double chartMinY, double chartMaxY) {
    // Para triglicéridos, la zona normal va desde 0 (chartMinY) hasta maxSafeZone
    double chartRange = chartMaxY - chartMinY;

    // Asegurar que las zonas estén dentro del rango visible
    double visibleMinSafe = chartMinY; // Siempre desde el mínimo del gráfico (0)
